...
public class VerifyDigitalSign {
    ...
    public static boolean verifySignature(String signedXml) throws Exception {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        factory.setNamespaceAware(true);
        DocumentBuilder builder = factory.newDocumentBuilder();
        Document document = builder.parse(new java.io.ByteArrayInputStream(signedXml.getBytes()));

        // Extract the Signature element
        NodeList signatureNodes = document.getElementsByTagNameNS(XMLSignature.XMLNS, "Signature");

        if (signatureNodes.getLength() == 0) {
            throw new Exception("No Signature element found in the XML");
        }
        Element signatureElement = (Element) signatureNodes.item(0);

        // Extract the X509Certificate
        NodeList certNodes = document.getElementsByTagName("X509Certificate");
        if (certNodes.getLength() == 0) {
            throw new Exception("No X509Certificate element found in the XML");
        }
        ...
        System.out.println(signature.validate(validateContext));
